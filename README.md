...
1. A value derived from $p$: The chance that, if the node is in level $k$, it's also
   in level $k + 1$. Specifically, its negated inverse log base 2 encoded as a fixed
   point real number with 6 bits of integer part and 26 bits of fractional part (see
   the `mkconstant.py` script);
...
- `mkconstant.py`, since `get_random_level` requires the negated inverse log base 2 of
  `p` encoded in fixed point with 6 bits of integer part and 26 bits of fractional
  part, this script does that work for you;
